...
void pwmInit() {
	...
	printf("2a\n");
    while(CM_PWMCTLbits.BUSY) printf(","); // Wait for generator to stop
	...
	printf("2a\n");
    while (!CM_PWMCTLbits.BUSY) printf(".");    // Wait for generator to start    
...
void pwmStop() {
    ...
    //PWM_CTLbits.MSEN1 = 0;  // Channel 1 in mark/space mode
    PWM_CTLbits.PWEN1 = 0;  // disable pwm
}
